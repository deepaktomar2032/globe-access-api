...
const requireEnvAnnotation: (...envs: string[]) => ClassDecorator = (
  ...envs: string[]
): ClassDecorator => {
...
const checkRequiredEnvs: () => boolean = (): boolean => {
  ...
  const missingEnvs: string[] = [...REQUIRED_ENV_KEYS].filter(
    (requiredEnv: string): boolean =>
      !process.env.hasOwnProperty(requiredEnv) || process.env[requiredEnv] === '',
  )
...
const checkRequiredEnvsByName = (...envs: string[]): boolean => {
  if (process.env.NODE_ENV && process.env.NODE_ENV === 'test') {
    return
  }
  const missingEnvs: string[] = envs.filter(
    (requiredEnv: string): boolean =>
      !process.env.hasOwnProperty(requiredEnv) || process.env[requiredEnv] === undefined,
  )
...
export {
  REQUIRED_ENV_KEYS,
  requireEnvAnnotation as RequireEnv,
  checkRequiredEnvs,
  checkRequiredEnvsByName,
  addEnvRequirements,
}
